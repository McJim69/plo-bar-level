	...
	<?php
		...
		<div class="d-flex justify-content-center mb-3 d-print-none">
			<div class="bg-dark p-2 rounded d-flex align-items-center gap-2 shadow-sm">
				<button class="btn btn-sm btn-outline-light" onclick="jump('?page=<?php echo ($p-1); ?>&barangay=<?php echo isset($_GET["barangay"]) ? urlencode($_GET["barangay"]) : ''; ?>&ato=<?php echo isset($_GET["ato"]) ? urlencode($_GET["ato"]) : ''; ?>')" <?php if($p <= 1) echo "disabled"; ?>>
					<i class="fa-solid fa-chevron-left"></i> Previous
				</button>
				<select class="form-select form-select-sm w-auto" onchange="jump('?page='+this.value+'&barangay=<?php echo isset($_GET["barangay"]) ? urlencode($_GET["barangay"]) : ''; ?>&ato=<?php echo isset($_GET["ato"]) ? urlencode($_GET["ato"]) : ''; ?>')">
					<?php
				...
				<button class="btn btn-sm btn-outline-light" onclick="jump('?page=<?php echo ($p+1); ?>&barangay=<?php echo isset($_GET["barangay"]) ? urlencode($_GET["barangay"]) : ''; ?>&ato=<?php echo isset($_GET["ato"]) ? urlencode($_GET["ato"]) : ''; ?>')" <?php if($p >= $total_pages) echo "disabled"; ?>>
		...
		<?php
		echo "<div id='thumbnails' class='tomb-grid' style='margin-top:-5px'>";
		...
		while($rs=$ex->fetch(PDO::FETCH_BOTH)){
			...
			$bck="background:#375ba2";
			if($rs["ato"]==="Undecided")
				$bck="background:#ff6700";
			else if($rs["ato"]==="Sure-OT")
				$bck="background:#aba100";
			else if($rs["ato"]==="Dili-Ato")
				$bck="background:#d32727";
			...
			<div class='tomb card-glass' id='div_".$rs["0"]."' 
				...
				<div style='".$bck.";text-align:center;font-size:15px;padding:5px;'><b>".$rs["ato"]."</b></div>
				...
				<hr class='my-2'>
				<div class='small text-muted space-y-1' style='font-size: 13px;'>
					<div>HL Leader: <span style='font-weight:600;'>".$rsHL["vname"]."</span></div>
				</div>";
				...
				if(($_SESSION["access"]=="SuperAdmin") or ($_SESSION["access"]=="Admin")){
					...
					<div class='mt-3 d-flex gap-2 flex-wrap' style='visibility: hidden; transition: var(--transition);' id='div_controls_".$rs["0"]."'>
						<button type='button' onclick=\"return deletehlc('".$rs["0"]."','".$rs["2"]."');\" class='btn btn-sm btn-danger px-2 py-1' style='font-size:11px;'><i class='fa-solid fa-trash-can'></i> Remove</button>
...
<div id="toprint" style="width:1000px;margin:0 auto;display:none"><br><br>
	<DIV style="text-align:center;background:transparent;" id='spacer' ><br><br><br><br></div>
	<DIV style="text-align:center;background:transparent;display:none" id='header'>
		LIST OF HOUSEHOLD MEMBERS<BR>
	...
	<?PHP
		echo "<table width=100%>
			<tr>
				<th colspan=2>NAME OF HOUSEHOLD MEMBERS</th>
				<th>IDN</th>
				<th>SEX</th>
				<th>AGE</th>
				<th>PREC</th>
		...
		";
		$stmt = $link->prepare("select * from hl_children hc, voters v where {$bar_filter} {$ato_filter} hc.vin=v.vin and v.city_mun=? order by v.vname LIMIT {$from},2000 ");
		$stmt->execute([$_SESSION["city_mun"]]);
		$ex1 = $stmt;
		...
		while($rs3=$ex1->fetch(PDO::FETCH_BOTH)){
			...
			echo"' >
				...
				<td style='font-size:10px'>".$rs3["ato"]."</td>
				<td style='font-size:10px'>".htmlspecialchars($leader_name)."</td>
				<td style='font-size:10px'>".$rs3[3]."</td>
			</tr>";
			$ii++;
		}
		echo"</table><br><br>";
