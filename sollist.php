...
<div class="container my-4">
		...
		<div class="card-glass p-3">
			<div class="row g-3 align-items-center">
				...
				<div class="col-md-5 d-flex gap-2">
					...
					<button type="button" class="btn btn-success flex-fill" onclick="add();"><i class="fa-solid fa-plus"></i> Add Dili Ato</button>
	...
	<div id="thumbnails" class="tomb-grid" style="margin-top:-10px">
		<?php
			...
			$stmt = $link->prepare("select * from sollist hc, voters v where {$bar} hc.vin=v.vin and v.city_mun=? order by v.vname LIMIT {$from},{$to} ");
			$stmt->execute([$_SESSION["city_mun"]]);
			$ex = $stmt;
			if(isset($_POST["b_search"]) && $_POST["t_search"]!=""){
				$stmt = $link->prepare("select * from sollist hc, voters v where (v.vname LIKE CONCAT('%', ?, '%') or v.barangay LIKE CONCAT('%', ?, '%')) and {$bar} hc.vin=v.vin and v.city_mun=? order by v.vname LIMIT {$from},{$to} ");
			...
			while($rs=$ex->fetch(PDO::FETCH_BOTH)){
				...
				<div class='tomb card-glass' id='div_".$rs["vin"]."' 
					...
					<hr class='my-2'>
					<div class='small text-muted space-y-1' style='font-size: 13px;'>
						<div class='text-danger' style='font-weight: 600;'>Dili Ato</div>
					</div>";
					...
					if(($_SESSION["access"]=="SuperAdmin") or ($_SESSION["access"]=="Admin")){
						...
						<div class='mt-3 d-flex gap-2' style='visibility: hidden; transition: var(--transition);' id='div_controls_".$rs["vin"]."'>
							<button type='button' onclick=\"deletesol('".$rs["vin"]."')\" class='btn btn-sm btn-danger px-3'><i class='fa-solid fa-trash-can'></i> Remove</button>
						</div>";
					}
				echo "
				</div>";
...
<div id="toprint" style="width:1000px;margin:0 auto;display:none"><br><br>
	...
	<DIV style="text-align:center;background:transparent;display:none" id='header'>
		LISTAHAN SA MGA DILI INGON NATO<BR>
	...
	<?PHP
		echo "<table width=100%>
			<tr>
				<th colspan=2>VOTER'S NAME OF DILI ATO</th>
				<th>IDN</th>
				<th>SEX</th>
				<th>AGE</th>
				<th>PREC</th>
...
<script>		
	function addmce(id,row){
		...
		var url = "ajax/addsol.php?table="+table+"&vin="+vin;
		fetch(url)
			.then(response => response.text())
			.then(data => {
	...
	function deletesol(solno){		
		if(confirm("Are you sure?")){
			var url = "ajax/deletesol.php?solno="+solno;
			fetch(url)
				.then(response => response.text())
				.then(data => {
					if(data.trim() == "Success"){
				...
				.catch(err => console.error("Fetch error in deletesol:", err));
		}
	}
