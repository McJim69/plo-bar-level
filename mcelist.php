...
<div class="container my-4">
			...
			<div class="card-glass p-3">
				<div class="row g-3 align-items-center">
					...
					<div class="col-md-4 d-flex gap-2">
						...
						<button type="button" class="btn btn-success flex-fill" onclick="getVoters('');add();"><i class="fa-solid fa-plus"></i> Add BCE</button>
					</div>
					<div class="col-md-4">
		...
		<div id="grid" class="d-none">
			<div class="text-center mb-4">
				<h3>BARANGAY CORE GROUP (BCG) CHAIRMAN</h3>
				<h5>CITY OF <?php echo htmlspecialchars($_SESSION["city_mun"]); ?></h5>
				<?php if(isset($_GET["barangay"]) && $_GET["barangay"] != "All barangays" && $_GET["barangay"] != ""): ?>
					<h6>BARANGAY <?php echo htmlspecialchars($_GET["barangay"]); ?></h6>
				<?php endif; ?>
			</div>
		...
		<div class="mb-4 d-flex justify-content-between align-items-center">
			<h4 style="font-weight: 700; color: var(--text-main);">
				Barangay Core Group (BCG) Chairmen:
				<span class="text-danger" style="font-size: 16px;">
					<?php echo (isset($_GET["barangay"]) && $_GET["barangay"] != "All barangays" && $_GET["barangay"] != "") ? "Barangay " . htmlspecialchars($_GET["barangay"]) : "All Barangays"; ?>
				</span>
			</h4>
		</div>
					...
					while($rs=$ex->fetch(PDO::FETCH_BOTH)){
						...
						<div class='tomb card-glass' id='div_".$rs["vin"]."' 
							...
							<div class='small text-muted space-y-1' style='font-size: 13px;'>
								...
								<div>Address: <strong>".str_ireplace($value, $rep, $rs["address"]).", ".str_ireplace($value, $rep, $rs["barangay"])."</strong></div>
							</div>
							...
							// Render BCG Members list inside the card as small avatar list
							...
							<div class='mt-3 d-flex gap-2' style='visibility: hidden; transition: var(--transition);' id='div_controls_".$rs["vin"]."'>
								<button onclick=\"deletemce('".$rs["vin"]."')\" class='btn btn-sm btn-danger px-3'><i class='fa-solid fa-trash-can'></i> Remove</button>
								<button onclick=\"jump('mceinfo.php?mce=".$rs["vin"]."')\" type='button' class='btn btn-sm btn-dark px-3'><i class='fa-solid fa-eye'></i> View BCE</button>
							</div>
						</div>";
					$i++;
					}
