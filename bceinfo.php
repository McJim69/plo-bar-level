...
<div id="toprint" class="container d-none" style="max-width: 1000px;">
	<div class="d-flex align-items-center justify-content-between border-bottom pb-3 mb-4">
		<div class="d-flex gap-3 align-items-center">
			<?php
				if(file_exists("images/voters/".$_GET["bce"].".jpg")){
					echo "<img src='images/voters/".$_GET["bce"].".jpg' height='100' style='border-radius:6px;' />"; 
				}else{
			...
			<div>
				...
				<h4 class="text-muted">Barangay Core Group (BCG) Member</h4>
				<p class="mb-0 text-muted">Precinct: <?php echo $rsmce["precinct"]; ?> | <?php echo $rsmce["address"] . ", " . $rsmce["barangay"]; ?></p>
			</div>
		</div>
		<div>
			<h5>LIST OF PRECINCT LEADERS WITH HLs</h5>
		</div>
	</div>
	
	<?php
		$stmt = $link->prepare('select * from pl p, voters v where p.bcevin=? and p.vin=v.vin order by vname');
		$stmt->execute([$_GET["bce"]]);
		$ex1 = $stmt;
		$i=1;
			...
			<div class='mb-4'>
				<table class='table table-bordered table-sm mb-0'>
					<thead class='table-dark'>
						<tr>
							<th colspan='2'>NAME OF PRECINCT LEADER & HLS</th>
							<th>IDN</th>
							<th>SEX</th>
							<th>AGE</th>
							<th>PRECINCT</th>
					...
					<tbody>
						...
						$stmt = $link->prepare('select * from hl h, voters v where h.plvin=? and h.vin=v.vin order by v.vname');
						$stmt->execute([$rs3["vin"]]);
						$ex_m = $stmt;
						$iii=1;
	...
	<div class="container my-4">
		...
		<div class="card-glass mb-4 text-white" style="background: linear-gradient(135deg, #991b1b 0%, #7f1d1d 100%); border: none;">
			<div class="row align-items-center g-4">
				<div class="col-md-3 text-center">
					<?php
						if(file_exists("images/voters/".$_GET["bce"].".jpg")){
							echo "<img style='border: 3px solid rgba(255,255,255,0.3); border-radius: var(--radius-md); max-width: 100%; max-height: 220px; object-fit: cover;' src='images/voters/".$_GET["bce"].".jpg?".date("h:i:s")."' />"; 
						}else{
				...
				<div class="col-md-9">
					...
					<h5 style="color: rgba(255,255,255,0.8); font-weight: 500;">BARANGAY CORE GROUP (BCG) MEMBER</h5>
					...
					<div class="row g-2 mb-3" style="font-size: 14px;">
						...
						<div class="col-sm-6">Total Precinct Leaders: <strong class="text-warning"><?php echo $pl; ?></strong></div>
					</div>
					
					<div id="d_controls" class="d-flex gap-2 flex-wrap">
						<button class="btn btn-success btn-sm" onclick="add()"><i class="fa-solid fa-user-plus"></i> Add Precinct Leader</button>
		...
		<div class="mb-4">
			<h4 style="font-weight: 700; color: var(--text-main);">
				Precinct Leaders (PL) belonged to <span class="text-danger"><?php echo htmlspecialchars($rsmce["vname"]); ?></span>:
			</h4>
		</div>
				...
				<?php
					...
					while($rs=$ex->fetch(PDO::FETCH_BOTH)){
						...
						<div class='tomb card-glass' id='div_".$rs["vin"]."' 
							...
							<img onclick=\"jump('plinfo.php?pl=".$rs["vin"]."')\" src='$img_src?".date("h:i:s")."' style='cursor:pointer;' />
							...
							<div class='mt-3 d-flex gap-2' style='visibility: hidden; transition: var(--transition);' id='div_controls_".$rs["vin"]."'>
								<button onclick=\"deletepl('".$rs["vin"]."',".$i.")\" class='btn btn-sm btn-danger px-3'><i class='fa-solid fa-trash-can'></i> Remove</button>
								<button onclick=\"jump('plinfo.php?pl=".$rs["vin"]."')\" type='button' class='btn btn-sm btn-dark px-3'><i class='fa-solid fa-eye'></i> HL Records</button>
							</div>
						</div>";
						$i++;
					}
					if(isset($_GET["add"]) && $_GET["add"] == "auto"){
						echo "<script>$('#bAddPL').click()</script>";
					}
				?>
			</div>
		</form>
...
<script>
	function addmce(id,row){
		var vin=id;
		var url = "ajax/addpl.php?bcevin="+<?php echo $bce; ?>+"&vin="+vin;
		fetch(url)
			...
			.then(data => {
				if(data.trim() == "Success"){
					...
					window.location="plinfo.php?pl="+vin;
				} else {
					console.error("addpl returned:", data);
				}
			})
			.catch(err => console.error("Fetch error in addmce:", err));
	}
			
	function deletepl(pno,id){	
		if(confirm("Are you Sure?")){
			var url = "ajax/deletepl.php?vin="+pno;
			fetch(url)
				.then(response => response.text())
				.then(data => {
					if(data.trim() == "Success"){
						...
						console.error("deletepl returned:", data);
					}
				})
				.catch(err => console.error("Fetch error in deletepl:", err));
		}
	}
</script>
