...
<script> setActive("special"); </script>
...
<div class="container my-4">
		...
		<div class="card-glass p-3">
			<div class="row g-3 align-items-center">
				...
				<div class="col-md-5 d-flex gap-2">
					...
					<button type="button" class="btn btn-success flex-fill" onclick="add();"><i class="fa-solid fa-plus"></i> Add SOL</button>
	...
	<div id="thumbnails" class="tomb-grid" style="margin-top:-10px">
		<?php
			...
			$stmt = $link->prepare("select * from special hc, voters v where {$bar} hc.vin=v.vin and v.city_mun=? order by v.vname LIMIT {$from},{$to} ");
			$stmt->execute([$_SESSION["city_mun"]]);
			$ex = $stmt;
			if(isset($_POST["b_search"]) && $_POST["t_search"]!=""){
				$stmt = $link->prepare("select * from special hc, voters v where (v.vname LIKE CONCAT('%', ?, '%') or v.barangay LIKE CONCAT('%', ?, '%')) and {$bar} hc.vin=v.vin and v.city_mun=? order by v.vname LIMIT {$from},{$to} ");
			...
			while($rs=$ex->fetch(PDO::FETCH_BOTH)){
				...
				if(isset($_POST["b_upImg_".$rs["0"]])){
					move_uploaded_file($_FILES["b_file_".$rs["0"]]["tmp_name"], "images/voters/".$rs[1].".jpg");
					jump("");
				}

				$img_src = file_exists("images/voters/".$rs[1].".jpg") ? "images/voters/".$rs[1].".jpg" : "images/blank.jpg";
				...
				<div class='tomb card-glass' id='div_".$rs["0"]."' 
					...
					<div class='card-title' style='background:#d800ff;text-align:center;font-size:15px;color:white;padding:5px;'><b>Special Operation List</b></div>
					...
					if(($_SESSION["access"]=="SuperAdmin") or ($_SESSION["access"]=="Admin")){
						...
						<div class='mt-3 d-flex gap-2' style='visibility: hidden; transition: var(--transition);' id='div_controls_".$rs["0"]."'>
							<button type='button' onclick=\"deletespecial('".$rs["0"]."')\" class='btn btn-sm btn-danger px-3'><i class='fa-solid fa-trash-can'></i> Remove</button>
						</div>";
					}
				echo "
				</div>";
...
<script>		
	function addmce(id,row){
		...
		var url = "ajax/addspecial.php?table="+table+"&vin="+vin;
		fetch(url)
			.then(response => response.text())
			.then(data => {
	...
	function deletespecial(spno){		
		if(confirm("Are you sure?")){
			var url = "ajax/deletespecial.php?spno="+spno;
			fetch(url)
				.then(response => response.text())
				.then(data => {
					if(data.trim() == "Success"){
				...
				.catch(err => console.error("Fetch error in deletespecial:", err));
		}
	}
</script>
