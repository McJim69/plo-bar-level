...
<div class="container my-4">
		...
		<div class="card-glass p-3">
			<div class="row g-3 align-items-center">
				...
				<div class="col-md-6 d-flex gap-2">
					...
					<button type="button" class="btn btn-dark flex-fill" onclick="jump('hllist_grid.php?barangay=<?php echo urlencode($_GET["barangay"]);?>')"><i class="fa-solid fa-list"></i> List View</button>
	...
	<?php
		...
		if (isset($_POST["l_search"]) && $h_search != "") {
			$stmt_hl = $link->prepare("select * from voters v, pl p where (v.vname LIKE CONCAT('%', ?, '%') or v.barangay LIKE CONCAT('%', ?, '%') or v.precinct LIKE CONCAT('%', ?, '%') or v.address LIKE CONCAT('%', ?, '%')) and p.vin=v.vin and v.city_mun=? order by vname");
			$stmt_hl->execute([$h_search, $h_search, $h_search, $h_search, $_SESSION["city_mun"]]);
		} else {
			$stmt_hl = $link->prepare('select * from voters v, pl p where p.vin=v.vin and v.city_mun=? order by vname');
						...
						while($rs=$ex_hl->fetch(PDO::FETCH_BOTH)){
							...
							<div class='col-md-3 mb-4'>
								<div class='card h-100 shadow-sm' id='div_".$modal_i."' 
								  ...
								  <div class='card-body text-center' onclick=\"jump('plinfo.php?pl=".$rs["vin"]."')\" style='cursor:pointer'>
	...
	<div class="d-flex justify-content-center mb-3 d-print-none">
		<div class="bg-dark p-2 rounded d-flex align-items-center gap-2 shadow-sm">
			<button class="btn btn-sm btn-outline-light" onclick="jump('?page=<?php echo ($p-1); ?>&barangay=<?php echo isset($_GET["barangay"]) ? urlencode($_GET["barangay"]) : ''; ?>')" <?php if($p <= 1) echo "disabled"; ?>>
				<i class="fa-solid fa-chevron-left"></i> Previous
			</button>
			<select class="form-select form-select-sm w-auto" onchange="jump('?page='+this.value+'&barangay=<?php echo isset($_GET["barangay"]) ? urlencode($_GET["barangay"]) : ''; ?>')">
				<?php
			...
			<button class="btn btn-sm btn-outline-light" onclick="jump('?page=<?php echo ($p+1); ?>&barangay=<?php echo isset($_GET["barangay"]) ? urlencode($_GET["barangay"]) : ''; ?>')" <?php if($p >= $total_pages) echo "disabled"; ?>>
	...
	<div id="thumbnails" class="tomb-grid" style="margin-top:-5px">
		<?php
			...
			while($rs=$ex->fetch(PDO::FETCH_BOTH)){
				...
				$stmt = $link->prepare('select * from voters v, hl h where h.vin=? and h.plvin=v.vin');
				$stmt->execute([$rs["vin"]]);
				$rspl = $stmt->fetch(PDO::FETCH_BOTH);
				$pl_name = $rspl ? $rspl["vname"] : "NOT SET";
				$pl_vin = $rspl ? $rspl["vin"] : "";
				...
				<div class='tomb card-glass' id='div_".$rs["vin"]."' 
					...
					<div class='small text-muted space-y-1' style='font-size: 13px;'>
						...
						<div>Precinct Leader: <a style='color:#00940e; font-weight:600;' href='plinfo.php?pl=".$pl_vin."'>".str_replace($value, $rep, $pl_name)."</a></div>
					</div>";
					...
					if(($_SESSION["access"]=="SuperAdmin") or ($_SESSION["access"]=="Admin")){
						...
						<div class='mt-3 d-flex gap-2 flex-wrap' style='visibility: hidden; transition: var(--transition);' id='div_controls_".$rs["vin"]."'>
							<button type='button' onclick=\"deletehl('".$rs["vin"]."',".$i.")\" class='btn btn-sm btn-danger px-2 py-1' style='font-size:11px;'><i class='fa-solid fa-trash-can'></i> Remove</button>
							<button type='button' onclick=\"jump('hlinfo.php?hl=".$rs["vin"]."')\" class='btn btn-sm btn-success px-2 py-1' style='font-size:11px;'><i class='fa-solid fa-eye'></i> View Mem</button>
							<a href='hl-indiv.php?hl=".$rs["vin"]."&barangay=".$rs["barangay"]."' target='_blank' class='btn btn-sm btn-secondary px-2 py-1' style='font-size:11px;'><i class='fa-solid fa-print'></i> Print</a>
						</div>";
					}
				echo "
				</div>";
