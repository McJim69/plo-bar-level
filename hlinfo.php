<?php
	...
	$stmt = $link->prepare('select * from voters v, hl h where h.vin=? and v.vin=h.plvin ');
	$stmt->execute([$_GET["hl"]]);
	$ex2 = $stmt;
	$rspl_data=$ex2->fetch(PDO::FETCH_BOTH);
			...
			<div class="row align-items-center g-4">
				...
				<div class="col-md-9">
					...
					<div id="d_controls" class="d-flex gap-2 flex-wrap">
						...
						<button class="btn btn-outline-light btn-sm" onclick="jump('hllist.php?barangay=SAN%20PEDRO')"><i class="fa-solid fa-list"></i> HL List</button>
			...
			if ($birthDate[0] !== '0000' && !empty($birthDate[0])) {
				...
				if ($birthObj !== false) {
					$age = date_diff($birthObj, date_create('today'))->y;
						...
						$bck = "background:var(--accent-blue) !important; color:#fff;";
						if($rs["ato"] === "Undecided") {
							$bck = "background:#dc2626 !important; color:#fff;";
						} else if($rs["ato"] === "Sure-OT") {
							$bck = "background:#ca8a04 !important; color:#fff;";
						}
						...
						<div class='tomb card-glass' id='div_".$rs["vin"]."' 
							...
							<div class='text-center p-1 small rounded mb-2' style='$bck font-weight: 700; font-size: 13px; text-transform: uppercase;'>
								$ato_val
							</div>
							...
							<div class='mt-3 d-flex gap-2 justify-content-between align-items-center' style='visibility: hidden; transition: var(--transition);' id='div_controls_".$rs["vin"]."'>
								<button onclick=\"deletehlc('".$rs["vin"]."','".$rs["2"]."')\" class='btn btn-sm btn-danger px-2'><i class='fa-solid fa-trash-can'></i></button>
